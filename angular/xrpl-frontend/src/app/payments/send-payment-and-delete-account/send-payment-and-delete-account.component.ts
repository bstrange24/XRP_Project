...
interface SendPaymentAndDeleteAccountApiResponse {
  status: string;
  message: string;
  result: {
...
export class SendPaymentAndDeleteAccountComponent implements OnInit {
  account: string = '';
  senderSeed: string = '';
  ...
  async sendPaymentAndDeleteAccount(): Promise<void> {
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.http.post<SendPaymentAndDeleteAccountApiResponse>(
      ...
      if (response && response.status === 'success') {
        ...
        this.errorMessage = 'Failed to send payment and delete account.';
        this.snackBar.open(this.errorMessage, 'Close', { duration: 5000 });
      }

      this.isLoading = false;
    } catch (error: any) {
      this.errorMessage = 'Error sending payment and deleting account: ' + (error.message || 'Unknown error');
      this.snackBar.open(this.errorMessage, 'Close', { duration: 5000 });
      this.isLoading = false;
    }
  }
