...
MAX_NUMBER_OF_TRADES_TO_EXECUTE = 5
...
class RealTimeArbitrageBot:
    def __init__(self, websocket_url=WEB_SOCKET_URL, spread_percent=0.001):
        ...
        self.min_profit = 0.00005  # Lowered to capture smaller opportunities
        ...
        for line in lines_response.result.get("lines", []):
            if line["currency"] == currency and line["account"] == issuer:
                ...
                if balance > 0.0001:  # Sell if above a small threshold
                    ...
                    if bid:
                        xrp_amount = Decimal(str(balance)) * Decimal(str(bid)) * Decimal('1.03')  # 2% buffer
    ...
    async def check_liquidity(self, taker_gets, taker_pays, amount, client):
        try:
            book_offers = await client.request(BookOffers(
                taker_gets=taker_gets,
                taker_pays=taker_pays,
            ...
            for offer in offers:
                if isinstance(taker_gets, XRP):
                    available = float(offer["TakerGets"]) / 1_000_000  # XRP
                else:
                    available = float(offer["TakerGets"]["value"])  # Token
    ...
    async def fetch_top_tokens(self, client, max_tokens=10):
        ...
        try:
            ...
            for token_key in top_tokens:
                ...
                # Check liquidity for XRP -> Token (buying token with XRP)
                min_amount_needed = float(self.trade_amount) / best_ask if best_ask > 0 else float('inf')
                has_liquidity = await self.check_liquidity(
                    XRP(),
                ...
                if not has_liquidity:
                    print(f"Skipping {token_key} due to insufficient XRPL liquidity")
                    continue

                verified_tokens.append(token_key)
                print(f"Verified {token_key} has pricing data and sufficient liquidity")
    ...
    async def execute_triangular_trade(self, token1_key, token2_key):
        ...
        try:
            ...
            async with AsyncWebsocketClient(self.websocket_url) as client:
                ...
                token1_amount = Decimal(str(self.trade_amount)) / p["buy_token1"]
                token1_amount_buffered = token1_amount / Decimal('1.02')  # 10% buffer to increase price
                ...
                token2_amount = xrp_from_token1 / p["buy_token2"]
                token2_amount_buffered = token2_amount / Decimal('1.03')  # 10% buffer to increase price
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                if not triangular_pairs:
                    print("No pairs found, retrying in 60 seconds...")
                    await asyncio.sleep(15)
                    continue
