...
#         self.response.is_successful = MagicMock(return_value=True)
...
#             'hash': 'somehashvalue'
#         }
#         self.assertTrue(validate_xrpl_response_data(response))
#
#     def test_invalid_response_no_meta(self):
#         self.response.result = {'hash': 'somehashvalue'}
#         self.assertFalse(validate_xrpl_response_data(self.response))
#
#     def test_invalid_response_no_hash(self):
#         self.response.result = {'meta': {'TransactionResult': 'tesSUCCESS'}}
#         self.assertFalse(validate_xrpl_response_data(self.response))
#
#     def test_invalid_transaction_result(self):
#         self.response.result['meta']['TransactionResult'] = 'tefFAILURE'
#         self.assertFalse(validate_xrpl_response_data(self.response))
#
#     def test_response_not_successful(self):
#         self.response.is_successful = MagicMock(return_value=False)
#         self.assertFalse(validate_xrpl_response_data(self.response))
#
#     def test_none_response(self):
#         self.assertFalse(validate_xrpl_response_data(None))

class TestGetAccountInfo(unittest.TestCase):
    def setUp(self):
        self.factory = RequestFactory()
        self.view = Accounts.as_view({'get': 'get_account_info'})
    ...
    # @patch('your_module.process_transaction_error')
    # @patch('your_module.convert_drops_to_xrp')
    # @patch('your_module.get_account_reserves')
    # @patch('your_module.create_wallet_info_response')
    # def test_get_account_info_success(self, mock_create_wallet_info_response, mock_get_account_reserves, mock_convert_drops_to_xrp, mock_process_transaction_error, mock_validate_xrpl_response_data, mock_prepare_account_data, mock_does_account_exist, mock_validate_xrp_wallet, mock_get_xrpl_client):
    #     # Setup mocks
    #     mock_get_xrpl_client.return_value = MagicMock()
    #     mock_validate_xrp_wallet.return_value = True
    #     mock_does_account_exist.return_value = True
    #     mock_get_account_reserves.return_value = (10, 2)
    #     mock_validate_xrpl_response_data.return_value = True
    #     mock_convert_drops_to_xrp.return_value = 10.0
    #     mock_create_wallet_info_response.return_value = {"status": "success"}
    #
    #     # Call the function
    #     response = Accounts.get_account_info(None, 'rEXAMPLE')
    #
    #     # Validate the response
    #     self.assertEqual(response, {"status": "success"})
    #
    # @patch('your_module.get_xrpl_client')
    # @patch('your_module.validate_xrp_wallet')
    # def test_get_account_info_invalid_wallet(self, mock_validate_xrp_wallet, mock_get_xrpl_client):
    #     # Setup mocks
    #     mock_validate_xrp_wallet.return_value = False
    #
    #     # Call the function
    #     response = Accounts.get_account_info(None, 'invalid_wallet')
    #
    #     # Validate the response
    ...
    def test_get_account_info_client_initialization_failed(self, mock_get_xrpl_client):
        ...
        request = self.factory.get('/xrpl/account_info', {'account': self.account})

        # Call the function
        response = self.accounts_view.get_account_info(request)
    ...
    #
    #     # Call the function
    #     response = self.accounts_view.get_account_info(request, account=self.account)
    #
    #     # Validate the response
    #     self.assertEqual(response.status_code, 500)
    #     self.assertEqual(response.data['status'], 'error')
