...
from ..utilities.utilities import get_xrpl_client, total_execution_time_in_millis, validate_xrp_wallet, \
    validate_xrpl_response_data, count_xrp_received, validate_response
    ...
    def get_account_trust_lines(self, request):
        ...
        try:
            ...
            while True:
                ...
                account_lines_response = self.client.request(account_lines_info)
                validate_response(account_lines_response, "Failed to fetch account trustlines")
...
class SetTrustLines(BaseXRPLView):
    ...
    def set_trust_line(self, request):
        ...
        try:
            ...
            issuer_wallet_balance = get_balance(issuer_address, self.client)
            logger.info(f"Sender wallet retrieved: {sender_wallet.classic_address} Sender Address Balance: {sender_wallet_balance} Issuer Address Balance: {issuer_wallet_balance}")
            ...
            logger.info(f" Base Reserve: {base_reserve}, Reserve Increment: {reserve_increment}, Minimum reserve: {min_balance}")
            ...
            sender_wallet_account_info_response = self.client.request(sender_wallet_account_info)
            validate_response(sender_wallet_account_info_response, "Failed to set account trustlines")
            ...
            trust_set_tx = create_trust_set_transaction(currency_code, limit_drops, issuer_address, sender_wallet.classic_address, sequence_number, fee, current_ledger)

            logger.info("signing and submitting the transaction, awaiting a response")
            validated_tx_response = submit_and_wait(trust_set_tx, self.client, sender_wallet)
            validate_response(validated_tx_response, "Failed to set account trustlines in submit_and_wait")

            count_xrp_received(validated_tx_response.result, issuer_address)

            tx_hash = validated_tx_response.result['hash']
            logger.info(f"Transaction validated with hash: {tx_hash} in ledger: {validated_tx_response.result['ledger_index']}")
            logger.info(f"Trust line created: {validated_tx_response.result} for account {issuer_address}")
            logger.info(f"Sender Address Balance: {get_balance(sender_wallet.classic_address, self.client)}")
            logger.info(f"Issuer Address Balance: {get_balance(issuer_address, self.client)}")
...
class RemoveTrustLine(BaseXRPLView):
    ...
    def remove_trust_line(self, request):
        ...
        try:
            ...
            offers = self.client.request(prepare_account_offers(sender_wallet.classic_address)).result["offers"]
            if any(("currency" in offer["taker_gets"] and offer["taker_gets"]["currency"] == currency_code) or
                   ("currency" in offer["taker_pays"] and offer["taker_pays"]["currency"] == currency_code) for offer in offers):
                raise XRPLException(error_response("Outstanding offers prevent trustline removal"))

            # Check trustline eligibility
            response = self.client.request(prepare_account_lines(sender_wallet.classic_address, None))
            lines = [line for line in response.result["lines"] if line["currency"] == currency_code and line["account"] == issuer_address]
            if not lines:
            ...
            # Fetch current fee
            fee_response = self.client.request(Fee())
            validate_response(fee_response, "Failed to fetch fees")
            ...
            validate_response(account_info_response, "Failed to fetch account data")
            ...
            validate_response(validated_tx_response, "Failed to remove account trustlines in submit_and_wait")
            count_xrp_received(validated_tx_response.result, issuer_address)
